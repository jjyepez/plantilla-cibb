	...
	**/ 
	...
	switch ( $operacion ){
		...
		case 'e': // se intenta EDITAR un registro con id = $id_registro .... jjy
			$titulo 			 = "Editar Rol ".$informacion_rol['nombre_rol'];
		...
		case 'm': // se intenta MOSTRAR un registro con id = $id_registro .... jjy
			$titulo 			 = "Mostrando Rol ".$informacion_rol['nombre_rol'];
			$instrucciones = "Los campos no son editables en la operación Mostrar.";
			$codigo_rol=$informacion_rol['codigo_rol'];
		...
		case 'errorn': // para manejar la vista cuando se tiene un error con un registro nuevo CV
		$titulo 			 = "Error ";
		$instrucciones = " Verifique la información ingresada.<br>";
		$editable = TRUE;	
...
?>
	...
	<hr>
			...
			<?php 
				...
				if($operacion=='n' || $operacion=='errorn'){
				$parametros = array(
				...
				<tr>
					<td>
						<?php					
								...
								if($operacion=='error' || $operacion=='errorn'){
									$parametros['valor_inicial']   = set_value('descripcion_rol');
								}
							?>
		...
		<?php if($operacion=='e'|| $operacion=='n' || $operacion=='error'|| $operacion=='errorn'){ ?>
		...
		<?php if($operacion=='m'){ ?>
				<?=html_etiqueta('Permisos asociados al Rol')?>
				<table class="ancho-50-porciento">
					<tr>	
						<td id="permiso-rol">								
						</td>
					</tr>
...
var operacion="<?=$operacion?>";
	...
	if(operacion=='m'){
		var codigo_rol="<?=$codigo_rol?>";

			$( document ).ready(function() {
				mostrar_rol(codigo_rol);
			});	
