...
//# @helper Control-Usuarios 1.0b - Sep 2013
/**
...
if( ! function_exists('validar_usuario_contrasena') ) {
  ...
  function validar_usuario_contrasena( $nombre_usuario, $contrasena ){
    ...
    $rsp = comprobar_tablas_usuarios( TRUE );
    ...
    **/
    ...
    $info_usuario = informacion_usuario( $nombre_usuario );

    if ( $nombre_usuario == $info_usuario['nombre_usuario'] && $contrasena == $info_usuario['contrasena'] ) { 
      $salida['rsp'] = 'OK'; 
      $salida['codigo_usuario'] = $info_usuario['codigo_usuario'];
    } else {
      $salida['rsp'] = 'ERROR'; 
...
if( ! function_exists( 'usuario_habilitado_para' ) ) {
  ...
  function usuario_habilitado_para( $parametros ){
    ...
    extract( $parametros );
    //*****************************
    if(trim($codigo_usuario)==""){
      extract( datos_usuario_actual() );
    } // se extraen los datos de usuario antes de los parametros por si se deben sobrescribir .... jjy
...
function codigo_modulo_segun_id( $id_modulo ){
  ...
  $info_modulo = $CI->control_usuarios_m->informacion_modulo ( $parametros );
  if ( $info_modulo != "" ){
    $codigo_modulo = $info_modulo['codigo_modulo'];
  }

  return $codigo_modulo;
}

function informacion_modulo_segun_id( $id_modulo ){
  $CI       =& get_instance();
  $CI->load->model( 'seguridad/control_usuarios_m' );
  ...
  $info_modulo_salida = array();

  $info_modulo = $CI->control_usuarios_m->informacion_modulo ( $parametros );
  if ( $info_modulo != "" ){
    $info_modulo_salida = $info_modulo;
  }
  
  return $info_modulo;
}

function codigo_permiso_segun_id( $id_permiso ){
  // ESTE MÉTODO DEBE SER ELIMINADO ... existe sólo por compatibilidad con ciertos módulos (alimentos/usuarios);
  return codigo_permiso_segun_nombre( $id_permiso );
}

function codigo_permiso_segun_nombre( $nombre_permiso ){
  $CI       =& get_instance();
  $CI->load->model( '../../aplicacion_base/models/seguridad/control_usuarios_m' );
  ...
  $info_permiso = $CI->control_usuarios_m->informacion_permiso ( $parametros );
  if ( $info_permiso != "" ){
    $codigo_permiso = $info_permiso['codigo_permiso'];
  }
